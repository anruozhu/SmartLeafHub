			...
			<u-card :title="title1" :title-size="title_size">
				<view class="" slot="body">
					<view class="u-body-item u-flex u-border-bottom u-col-between u-p-t-0" style="display: flex;">
						<view class="img">
							<image src="../../static/温度.png" style="width: 100rpx;height: 100rpx;margin-top: 0px;margin-left: 2px;"></image>
						</view>
						<view class="yujing">
		...
		methods:{
				...
				uni.request({
					...
					success: (res) => {
						if(res.data.code*1==200){
							...
							this.trend[0]=res.data.data[6].airTemperature
							this.trend[1]=res.data.data[5].airTemperature
							this.trend[2]=res.data.data[4].airTemperature
							this.trend[3]=res.data.data[3].airTemperature
							this.trend[4]=res.data.data[2].airTemperature
							this.trend[5]=res.data.data[1].airTemperature
							this.trend[6]=res.data.data[0].airTemperature
						}
					},
					fail: (err) => {
						console.error('请求失败',err);
			...
			changeSwitch(e){
				...
				if(this.checked1==false){
					...
					this.bin=false
					const token=uni.getStorageSync('login_user_token');
					let url=`http://139.155.150.150:8070/secure/WindAndPumpctl?f_level=${this.value}&f_mode=1`;
					uni.request({
						url:url,
						method:"GET",
						header:{
			...
			changePicker(e){
				// console.log(e)
				const token=uni.getStorageSync('login_user_token');
				let url=`http://139.155.150.150:8070/secure/WindAndPumpctl?f_level=${this.value}&f_mode=1&p_c_state=0&p_p_state=0`;
				uni.request({
					url:url,
					method:"GET",
					header:{
